...
# это все конечно зашибись но нужно посмотреть векторные алгоритмы
...
async def execute() -> None:
    ...
    while True:
        ...
        face = asyncio.create_task(face_recognition_full_face_v3(gray))
        profile = asyncio.create_task(face_recognition_profile_v3(gray))

        result_face = await face
        log_get_time(start_time, 'Face search in full view time: ')
        result_profile = await profile
        log_get_time(start_time, 'Face search in profile time:   ')
        we_check_that_the_face_is_recognized_v2(result_face, result_profile, img, gray)

        cv2.imshow('video', img)
        log_get_time(start_time, 'Total execution time:          ')
...
async def face_recognition_full_face_v3(gray):
    return faceCascadeFront.detectMultiScale(gray, scaleFactor=1.2, minNeighbors=5, minSize=(10, 10), )


async def face_recognition_profile_v3(gray):
    return faceCascadeProfile.detectMultiScale(gray, scaleFactor=1.2, minNeighbors=5, minSize=(10, 10), )
...
def log_get_time(start_time: float, task_name: str) -> None:
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f'{task_name}: {elapsed_time}')
